...
public class CPU {
    ...
    public void step(CpuMemory m){
        ...
        Map<String,MemWd> labels = Memory.instance();
        ExeCond exeCond = ExeCond.instance();
        ...
        long testLong;
        if (inst.isBranch()) {
            if (inst.getCondCode() == CondCode.Empty) {
                registers[PC] = labels.get(inst.getLabel()).getAddress();
            }
            else {
                if (exeCond.getCondStatus(inst.getCondCode())) {
                    registers[PC] = labels.get(inst.getLabel()).getAddress();
                }
            }
        }
